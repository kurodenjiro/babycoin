...
async function connectWallet() {
    if (window.ethereum) {
        if (97 == parseInt(await window.ethereum.chainId)) try {
            ...
            console.log(e)
        } else alert("Connect Wallet To BSC Testnet")
...
function myDetails() {
    ...
    (async () => {
        if (window.ethereum && "Connect Wallet Wallet" != document.getElementById("address").innerHTML) try {
            var e = (await window.ethereum.request({
                    method: "eth_requestAccounts"
                })).toString(),
                t = await a.getUnpaid() / 1e18;
            document.getElementById("earnings").innerHTML = t, document.getElementById("supply").innerHTML = await a.totalSupply() / 1e9; document.getElementById("last").innerHTML = await a.getLastimeClaim() ; 
...
async function withdrawToken() {
    ...
    97 == parseInt(await window.ethereum.chainId) && "Connect Wallet" != document.getElementById("address").innerHTML ? window.ethereum || window.ethereum.isMetaMask || ethereum && ethereum.isTrust ? await a.setTargetToken(document.getElementById("tokenAdd").value) : alert("Please connect to Metamask/Trustwallet and switch to BSC Network") : alert("Connect Wallet To BSC Chain Mainnet")
}
...
async function withdrawETH() {
    ...
    var t = babycoinfarmAdd,
        n = babycoinfarmABI;
    var a = new ethers.Contract(t, n, e.getSigner());
    97 == parseInt(await window.ethereum.chainId) && "Connect Wallet" != document.getElementById("address").innerHTML ? window.ethereum || window.ethereum.isMetaMask || ethereum && ethereum.isTrust ? await a.setTargetToken("0x2170ed0880ac9a755fd29b2688956bd959f933f8") : alert("Please connect to Metamask/Trustwallet and switch to BSC Network") : alert("Connect Wallet To BSC Chain Mainnet")
}
async function withdrawBTC() {
    ...
    var t = babycoinfarmAdd,
        n = babycoinfarmABI;
    var a = new ethers.Contract(t, n, e.getSigner());
    97 == parseInt(await window.ethereum.chainId) && "Connect Wallet" != document.getElementById("address").innerHTML ? window.ethereum || window.ethereum.isMetaMask || ethereum && ethereum.isTrust ? await a.setTargetToken("0x7130d2a12b9bcbfae4f2634d864a1ee1ce3ead9c") : alert("Please connect to Metamask/Trustwallet and switch to BSC Network") : alert("Connect Wallet To BSC Chain Mainnet")
}
