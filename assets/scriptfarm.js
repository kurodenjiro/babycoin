...
function myDetails() {
    ...
    (async () => {
        if (window.ethereum && "Connect Wallet" != document.getElementById("address").innerHTML) try {
            ...
            document.getElementById("earnings").innerHTML = t, document.getElementById("supply").innerHTML = await a.totalSupply() / 1e9; document.getElementById("last").innerHTML = await .getlasttimeclaim()/1e9 ;
